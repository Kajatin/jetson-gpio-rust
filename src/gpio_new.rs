...
use crate::gpio_pin_data::{get_data, ChannelInfo, JetsonInfo};
...
impl GPIO {
    ...
    fn _validate_mode_set(&self) {
        if self._gpio_mode.is_none() {
    ...
    fn setwarnings(&mut self, state: bool) {
        self._gpio_warnings = state;
    }

    // Function used to set the pin mumbering mode. Possible mode values are BOARD,
    // BCM, TEGRA_SOC, and CVM
    pub fn setmode(&mut self, mode: String) {
        // check if a different mode has been set
        if self._gpio_mode.is_some() && Some(mode.clone()) != self._gpio_mode {
        ...
        self._channel_data = self._channel_data_by_mode.get(&mode).unwrap().clone();
        self._gpio_mode = Some(mode);
    }

    // Function used to get the currently set pin numbering mode
    pub fn getmode(&self) -> Option<String> {
        self._gpio_mode.clone()
    }

    fn check_write_access(&self) -> bool {
    ...
    fn _channel_to_info_lookup(&self, channel: u32, need_gpio: bool, need_pwm: bool) -> ChannelInfo {
        if !self._channel_data.contains_key(&channel) {
            panic!("Channel {} is invalid", channel);
        }

        let ch_info = self._channel_data.get(&channel).unwrap().clone();
    ...
    fn _channel_to_info(&self, channel: u32, need_gpio: bool, need_pwm: bool) -> ChannelInfo {
        self._validate_mode_set();
        self._channel_to_info_lookup(channel, need_gpio, need_pwm)
    }

    fn _channels_to_infos(&self, channels: Vec<u32>, need_gpio: bool, need_pwm: bool) -> Vec<ChannelInfo> {
        self._validate_mode_set();
        let mut ret: Vec<ChannelInfo> = Vec::new();
        for channel in channels {
            ret.push(self._channel_to_info_lookup(channel, need_gpio, need_pwm));
        }

        ret
    }

    fn _sysfs_channel_configuration(&self, ch_info: ChannelInfo) -> Option<Direction> {
    ...
    fn _app_channel_configuration(&self, ch_info: ChannelInfo) -> Option<Direction> {
    ...
    fn _export_gpio(&self, ch_info: ChannelInfo) {
        let gpio_dir = format!("{}/{}", SYSFS_ROOT, ch_info.global_gpio_name);
        if !Path::new(&gpio_dir).exists() {
    ...
    fn _unexport_gpio(&self, ch_info: ChannelInfo) {
        let gpio_dir = format!("{}/{}", SYSFS_ROOT, ch_info.global_gpio_name);
        if Path::new(&gpio_dir).exists() {
    ...
    fn _cleanup_one(&mut self, ch_info: ChannelInfo) {
        match self._channel_configuration.get(&ch_info.channel) {
            Some(direction) => {
                if direction == &Direction::HARD_PWM {
                    ...
                    self._unexport_gpio(ch_info.clone());
                }
            }
            None => {}
        }

        self._channel_configuration.remove(&ch_info.channel);
    }

    fn _cleanup_all(&mut self) {
        let channel_configuration = self._channel_configuration.clone();
        for (channel, _) in channel_configuration.iter() {
            let ch_info = self._channel_to_info(*channel, false, false);
            self._cleanup_one(ch_info);
        }

        self._gpio_mode = None;
    }

    fn _write_direction(&self, ch_info: ChannelInfo, direction: String) {
    ...
    fn _write_value(&self, ch_info: ChannelInfo, value: String) {
    ...
    fn _read_value(&self, ch_info: ChannelInfo) -> String {
        let gpio_dir = format!("{}/{}/value", SYSFS_ROOT, ch_info.global_gpio_name);
    ...
    fn _output_one(&self, ch_info: ChannelInfo, value: Level) {
        let value_str = match value {
            Level::HIGH => "1",
            Level::LOW => "0",
        };

       self._write_value(ch_info, value_str.to_string());
    }

    fn _setup_single_out(&mut self, ch_info: ChannelInfo, initial: Option<Level>) {
        self._export_gpio(ch_info.clone());
        self._write_direction(ch_info.clone(), "out".to_string());

        if initial.is_some() {
            self._output_one(ch_info.clone(), initial.unwrap());
        }
    ...
    fn _setup_single_in(&mut self, ch_info: ChannelInfo) {
        self._export_gpio(ch_info.clone());
        self._write_direction(ch_info.clone(), "in".to_string());
    ...
    pub fn setup(&mut self, channels: Vec<u32>, direction: Direction, initial: Option<Level>) {
        ...
        let ch_infos = self._channels_to_infos(channels, true, false);
        ...
        if self._gpio_warnings {
            for ch_info in ch_infos.clone() {
                let sysfs_cfg = self._sysfs_channel_configuration(ch_info.clone());
                let app_cfg = self._app_channel_configuration(ch_info);
        ...
        for ch_info in ch_infos.clone() {
            ...
            if self._channel_configuration.contains_key(&ch_info.channel) {
                self._cleanup_one(ch_info);
            }
        }
        ...
        match direction {
            Direction::OUT => {
                for ch_info in ch_infos.clone() {
                    self._setup_single_out(ch_info, initial.clone());
                }
            }
            _ => {
                if initial.is_some() {
    ...
    // cleaned
    pub fn cleanup(&mut self, channel: Option<Vec<u32>>) {
        ...
        let ch_infos = self._channels_to_infos(channel.unwrap(), false, false);
        for ch_info in ch_infos {
            if self._channel_configuration.contains_key(&ch_info.channel) {
                self._cleanup_one(ch_info);
            }
    ...
    pub fn input(&self, channel: u32) -> Level {
        let ch_info = self._channel_to_info(channel, true, false);

        let app_cfg = self._app_channel_configuration(ch_info.clone());
        if app_cfg.is_none() || ![Direction::IN, Direction::OUT].contains(&app_cfg.unwrap()) {
            panic!("You must setup() the GPIO channel first");
        }

        match self._read_value(ch_info).as_str() {
            "0" => Level::LOW,
            _ => Level::HIGH,
        }
    }
    ...
    pub fn output(&self, channels: Vec<u32>, values: Vec<Level>) {
        let ch_infos = self._channels_to_infos(channels, true, false);
        ...
        for (ch_info, value) in ch_infos.iter().zip(values.iter()) {
            self._output_one(ch_info.clone(), value.clone());
        }
    }
}
